...
namespace Mirage.Generators.Names
{
    ...
    public sealed class NameAttribute : StringGeneratorBase
    {
        ...
        public NameAttribute(bool prefix = false, bool middleName = false, bool lastName = true, bool suffix = false)
            : base("", "")
        {
            Prefix = prefix;
        ...
        public override string Next(Random rand) => rand.Next<bool>() ? new MaleNameAttribute(Prefix, MiddleName, LastName, Suffix).Next(rand) : new FemaleNameAttribute(Prefix, MiddleName, LastName, Suffix).Next(rand);
    }
}